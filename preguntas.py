...
def pregunta_02():
    ...
    print(print(f"{df['life'].corr(df['fertility']):.4f}"))

    # Imprima la media de la columna `life` con 4 decimales.
    print(print(f"{df.life.mean():.4f}"))

    # Imprima el tipo de dato de la columna `fertility`.
    print(print(type(df.fertility)))

    # Imprima la correlación entre las columnas `GDP` y `life` con 4 decimales.
    print(f"{df['life'].corr(df['GDP']):.4f}")
...
def pregunta_03():
    ...
    y_life = df['life'].to_numpy()[:, np.newaxis]
    ...
    prediction_space = np.linspace(X_fertility.min(), X_fertility.max())[:, np.newaxis]
...
def pregunta_04():
    ...
    X_fertility, y_life, test_size=0.20, random_state=53,
    )
